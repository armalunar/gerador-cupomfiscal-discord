...
c.execute('''CREATE TABLE IF NOT EXISTS tickets (id INTEGER PRIMARY KEY, service_name TEXT, price REAL, responsible TEXT, user TEXT, notes TEXT, generated_at DATETIME)''')
conn.commit()
...
async def generate_and_send_ticket(ctx):
    ticket_image = Image.new('RGB', (600, 750), color='#e9e5cd')
    ...
    draw.text((10, 100), f'Reponsável: {ticket_info["responsible"]}', fill='black', font=font)
    draw.text((10, 130), f'Usuário: {ticket_info["user"]}', fill='black', font=font)

    # Adicione o traço separador abaixo dos dados do usuário
    draw.line([(10, 160), (590, 160)], fill='black', width=2)

    # Check if 'notes' key exists in ticket_info
    if 'notes' in ticket_info:
        draw.text((10, 170), 'NOTAS:', fill='black', font=font)
        draw.text((10, 200), ticket_info['notes'], fill='black', font=font)

    # Adicionar a hora exata que o ticket foi gerado
    current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    draw.text((10, 220), f'Gerado em: {current_time}', fill='black', font=font)

    phrase = "TL STORE systems"
    draw.text((10, 650), phrase, fill='black', font=font)

    c.execute("INSERT INTO tickets (id, service_name, price, responsible, user, notes, generated_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
              (ticket_info["id"], ticket_info["service_name"], ticket_info["price"], ticket_info["responsible"], ticket_info["user"], ticket_info.get('notes', ''), current_time))
    conn.commit()
...
async def list_tickets(ctx):
    c.execute("SELECT id, service_name, price, responsible, user, notes, generated_at FROM tickets")
    tickets = c.fetchall()
    ...
    if not tickets:
        ...
        for ticket in tickets:
            ...
            if ticket[5]:  # Check if 'notes' exist
                message += f"NOTAS: {ticket[5]}\n"
            message += f"Gerado em: {ticket[6]}\n"
